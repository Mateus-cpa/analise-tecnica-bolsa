...
def acao_com_preditivo(acao):
    ...
    #X = sm.add_constant(data['X']) # Adiciona uma constante para o intercepto
    ...
    # Ajusta uma linha (polinômio de grau 1)
    #coefficients = np.polyfit(X, y, 1)
    #slope = coefficients[0]
    #intercept = coefficients[1]
    #print(f"Slope: {slope}, Intercept: {intercept}")
    # Para prever: slope * novo_X + intercept

    #from sklearn.linear_model import LinearRegression
    #data = {'feature': [10, 20, 30, 40], 'target': [15, 25, 33, 42]}
    #df = pd.DataFrame(data)
    # Preparar dados para o modelo (reshape para Scikit-learn)
    #X = df[['feature']]
    #y = df['target']
    #model = LinearRegression()
    #model.fit(X, y)
